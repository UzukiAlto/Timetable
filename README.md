...
https://syncschedule.onrender.com/
...
1. **レスポンシブ対応:** スマホでの利用を想定し、gridの大きさやフォントサイズ、フッターなどを画面の大きさによって調整しました。
2. **授業の同期:** 同一の授業を複数のコマに配置する機能を実装しました。
3. **少ないページ更新:** Java Scriptのfetch APIを活用した非同期通信を用い、授業の追加、編集、削除の際にページ更新を挟まないストレスフリーなアプリを目指しました。
4. **Google ログイン:** Googleアカウントを用いてログインできます。
